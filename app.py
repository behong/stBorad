...
if not data.empty:
    ...
    with header_col2 if is_admin else header_col1:
        st.write("**제목**")
    with header_col3 if is_admin else header_col2:
        st.write("**내용**")
    with header_col4 if is_admin else header_col3:
        st.write("**카테고리**")
    with header_col5 if is_admin else header_col4:
        st.write("**날짜**")
    ...
    for idx, row in sorted_data.iterrows():
        ...
        with col2 if is_admin else col1:
            st.write(row["제목"])
        with col3 if is_admin else col2:
            # 내용이 길면 일부만 표시
            content_preview = row["내용"][:50] + "..." if len(str(row["내용"])) > 50 else row["내용"]
            st.write(content_preview)
        with col4 if is_admin else col3:
            st.write(row["카테고리"])
        with col5 if is_admin else col4:
